...
5. Detect the selected framework from `.myway/PROJECT.md` when `.myway/` exists.
6. If `.myway/` does not exist, use the requested framework or default to `backend-guided` for the current MVP.
7. Read the corresponding file under `frameworks/`.
8. Inspect only the application files relevant to the current step.
